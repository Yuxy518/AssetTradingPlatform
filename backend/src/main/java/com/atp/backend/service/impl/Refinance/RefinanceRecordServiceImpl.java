...
public class RefinanceRecordServiceImpl implements RefinanceRecordService {
	...
	public Map<String, String> refinanceRecord(Integer userId) {
		...
		for(RefinanceInfo refinanceInfo : list1) {
			list.add(new Record(refinanceInfo.getSubmitdate(), null, "申请转按揭", refinanceInfo.getProgress(), refinanceInfo.getRemarks()));
		}
		...
		for(RefinanceContract refinanceContract : list2) {
			list.add(new Record(refinanceContract.getSubmitTime(), null, "提交转按揭合同", refinanceContract.getProgress(), refinanceContract.getRemarks()));
		}
